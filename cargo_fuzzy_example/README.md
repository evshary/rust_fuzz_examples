...
This example lives in the `cargo_fuzzy_example/` folder. It contains a very
small Rust crate plus a `cargo-fuzz` target, and the setup is intentionally
simple so it can be used in a tutorial.
...
Run commands from inside `cargo_fuzzy_example/`:
...
`cargo_fuzzy_example::parse_port` with them. After enough mutations, it should discover
